...
function renderGudangList(){
  ...
  list.forEach(function(p){
    ...
    row.innerHTML='<div class="gd-gudang-em">'+(p.emoji||'📦')+'</div>'
      ...
      +'<div class="gd-gudang-meta">'+gdEscHtml(p.cat)+' &middot; '+gdEscHtml(p.brand||'-')+(p.berat?' &middot; '+p.berat+' g':'')+'</div>'
...
/* ===== MASUKKAN BARANG BARU KE SISTEM =====
   ...
   termasuk berat/isi bersih dalam gram. */
function gdTambahBarangBaru(){
  ...
  if(!berat||berat<=0){
    showToast('⚠️ Berat/isi bersih (gram) wajib diisi dan lebih dari 0');
    return;
  }
  ...
  renderGudangList();
  renderGudangHistory();
  showToast('✅ Barang "'+newProduct.name+'" ('+newProduct.berat+' g) berhasil dimasukkan ke sistem');
}
...
function gdSaveNewItemLog(p){
  ...
  list.push({id:p.id,name:p.name,emoji:p.emoji,cat:p.cat,brand:p.brand,berat:p.berat,price:p.price,stock:p.stock,date:new Date().toISOString(),by:gudangLoggedName||'gudang'});
...
function renderGudangHistory(){
  ...
  combined.slice().reverse().slice(0,30).forEach(function(entry){
    ...
    if(entry.type==='baru'){
      row.innerHTML='<span>'+(l.emoji||'📦')+' '+gdEscHtml(l.name)+' <span class="gh-new">BARU &middot; '+l.berat+' g &middot; stok awal '+l.stock+'</span></span>'
